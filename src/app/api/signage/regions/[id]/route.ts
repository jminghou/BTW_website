...
import { updateRegion, deleteRegion } from '@/lib/signage/db';
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await updateRegion(id, body);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (result.success) return NextResponse.json({ success: true, message: '區域已刪除' });
    return NextResponse.json({
      success: false,
